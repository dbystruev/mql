//+------------------------------------------------------------------+
//|                        Bystruev_2009_05_08_Long_Wave_inverse.mq4 |
//|                  Copyright © 2009, Denis Bystruev, 5 August 2009 |
//|                                           http://www.moeradio.ru |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2009, Denis Bystruev, 5-18 August 2009"
#property link      "http://www.moeradio.ru"

#define  MAGIC_NUMBER   320090818

// Если текущее падение или рост значительно больше, чем для энного количества
// предыдущих баров, нужно вставать в позицию против текущего бара

extern int     bars_to_track  =  1250; // количество баров, которые отслеживаем
extern int     lot_add        =  0;    // сколько прибавлять к лоту при пригрыше или убавлять при выигрыше
...
// вычисляем размер лота с учётом ограничений по минимуму и максимуму
double get_lot(double current_lot, int step = 0) {
   double lot_step = MarketInfo(Symbol(), MODE_LOTSTEP);
   double get_lot = current_lot + lot_step * step;
   get_lot = lot_step * MathRound(get_lot / lot_step);
   get_lot = MathMax(MarketInfo(Symbol(), MODE_MINLOT), get_lot);
   get_lot = MathMin(MarketInfo(Symbol(), MODE_MAXLOT), get_lot);
   return (get_lot);
}
...
int start() {
   ...
   if ((max < delta) && (min_delta < delta) && (OrdersTotal() < max_orders)) {
      if (Open[0] < Close[0]) {
         order_type = OP_SELL;   // растём вверх
         price = Bid;
         stoploss = High[iHighest(NULL, 0, MODE_HIGH)] + Point * MarketInfo(Symbol(), MODE_SPREAD);
         comment = comment + "Продаём лотов: " + DoubleToStr(lot, 2) + ", по цене: " + DoubleToStr(price, Digits);
      }
      if (Close[0] < Open[0]) {
         order_type = OP_BUY;    // убываем вниз
         price = Ask;
         stoploss = Low[iLowest(NULL, 0, MODE_LOW)];
