...
extern double   OrderDelta     =  50;      // difference between buy sell and stop sell orders
...
void buy_sell()
{
   ...
   if (sell_order >= 0)
   {
      stop_loss = Ask + (Ask - Bid);
      if (stop_loss < sell_price)
      {
         if (OrderSelect(sell_order, SELECT_BY_TICKET))
            if (MathAbs(OrderStopLoss()) > Zero)
               stop_loss = sell_price - StopLossFactor * (sell_price - stop_loss);
