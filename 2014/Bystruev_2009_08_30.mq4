...
extern   double   delta_lose_add    =  0.0;  // сколько добавлять к delta после проигрыша
extern   double   delta_lose_mult   =  2.0;  // на сколько умножать delta после проигрыша
extern   double   delta_win_add     =  0.0;  // сколько добавлять к delta после выигрыша
...
double   balance  =  0.0;  // текущий баланс
int      delta    =  0;    // уровни стоп-лосс и тейк-профит
int      ticket   =  -1;   // тикет ордера, которым торгуем
...
int start() {
   ...
   double   lot   =  MarketInfo(Symbol(), MODE_MINLOT);  // торгуем минимальным лотом
