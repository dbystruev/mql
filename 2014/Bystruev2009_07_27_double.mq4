...
int start() {
   ...
   if ((Ask < day_open_price - max_delta) || (day_open_price + max_delta < Bid) || (AccountEquity() < (1 - maximum_risk) * AccountBalance())) {
      close_all_orders();
      trade_allowed = FALSE;
      return;
   }
   ...
   switch (total_orders) {
      ...
      default:
         ...
         int mult = MathPow(mult_factor, total_orders + 2);
