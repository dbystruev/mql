...
void buy_sell()
{
   ...
   int buylimit_order = find_order(OP_BUYLIMIT, CurrentLot, buylimit_price);
   int selllimit_order = find_order(OP_SELLLIMIT, CurrentLot, selllimit_price);
   int buy_order = find_order(OP_BUY, CurrentLot, buy_price);
   int sell_order = find_order(OP_SELL, CurrentLot, sell_price);
   int Delta;

   if ((buy_order < 0) && (sell_order < 0) && (buystop_order < 0) && (sellstop_order < 0) && (buylimit_order < 0) && (selllimit_order < 0))
   {
      change_current_lot();
      sellstop_price = Point * MathFloor((Ask + Bid - OrderDelta * Point) / 2.0 / Point);
      buystop_price = sellstop_price + OrderDelta * Point;
      buylimit_price = sellstop_price;
      selllimit_price = buystop_price;
//      Print("sellstop_price = " + sellstop_price + "  buystop_price = " + buystop_price);
//      OrderSend(Symbol(), OP_BUYSTOP, CurrentLot, buystop_price, 0, 0.0, 0.0);
//      OrderSend(Symbol(), OP_SELLSTOP, CurrentLot, sellstop_price, 0, 0.0, 0.0);
      OrderSend(Symbol(), OP_SELLLIMIT, CurrentLot, selllimit_price, 0, 0.0, 0.0);
      OrderSend(Symbol(), OP_BUYLIMIT, CurrentLot, buylimit_price, 0, 0.0, 0.0);
      return;
   }
   ...
   if ((buylimit_order >= 0) && (selllimit_order >= 0))
   {
      Delta = MathMax(2 * MarketInfo(Symbol(), MODE_STOPLEVEL) + MarketInfo(Symbol(), MODE_SPREAD), MathRound((selllimit_price - buylimit_price) / Point));
      Print("Delta = " + Delta);
      buylimit_price = Point * MathFloor((Ask + Bid - Delta * Point) / 2.0 / Point);
      selllimit_price = buylimit_price + Delta * Point;
      update_limit_order(buylimit_order, buylimit_price);
      update_limit_order(selllimit_order, selllimit_price);
   ...
   {
      Delta = MathMax(2 * MarketInfo(Symbol(), MODE_STOPLEVEL) + MarketInfo(Symbol(), MODE_SPREAD), MathRound((buystop_price - sellstop_price) / Point));
      Print("Delta = " + Delta);
      sellstop_price = Point * MathFloor((Ask + Bid - Delta * Point) / 2.0 / Point);
      buystop_price = sellstop_price + Delta * Point;
   ...
   if (selllimit_order >= 0)
   {
      if (Ask < selllimit_price - OrderDelta * Point)
      {
         OrderDelete(selllimit_order);
      } else
         OrderSend(Symbol(), OP_BUYLIMIT, CurrentLot, selllimit_price - OrderDelta * Point, 0.0, 0.0, 0.0);
      return;
   }
   
   if (buylimit_order >= 0)
   {
      if (buylimit_price + OrderDelta * Point < Bid)
      {
         OrderDelete(buylimit_order);
      } else
         OrderSend(Symbol(), OP_SELLLIMIT, CurrentLot, buylimit_price + OrderDelta * Point, 0.0, 0.0, 0.0);
      return;
   }
...
void update_stop_order(int order_ticket, double price)
{
   if (OrderSelect(order_ticket, SELECT_BY_TICKET))
   {
      ...
      {
         case OP_BUYSTOP:
            if (price < OrderOpenPrice()) OrderModify(order_ticket, price, 0.0, 0.0, 0);
            break;
         case OP_SELLSTOP:
            if (OrderOpenPrice() < price) OrderModify(order_ticket, price, 0.0, 0.0, 0);
            break;
      }
   }
}
...
//| Update limit order                                               |
//+------------------------------------------------------------------+
void update_limit_order(int order_ticket, double price)
{
   if (OrderSelect(order_ticket, SELECT_BY_TICKET))
   {
      switch (OrderType())
