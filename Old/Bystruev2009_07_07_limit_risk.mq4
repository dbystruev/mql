...
extern   double   min_risk    =  0.01;

//+------------------------------------------------------------------+
//|                                Bystruev2009_07_07_limit_risk.mq4 |
//|         Copyright © 2009, Denis Bystruev, 24 June - 23 July 2009 |
//|                                           http://www.moeradio.ru |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2009, Denis Bystruev, 24 June - 23 July 2009"
#property link      "http://www.moeradio.ru"

#define  MAGIC_NUMBER   320090723
...
double next_lot(double lot) {
   lot = lot_mult * lot + lot_add * MarketInfo(Symbol(), MODE_LOTSTEP);
   if (max_risk * AccountBalance() / MarketInfo(Symbol(), MODE_MARGINREQUIRED) < lot)
      delta = next_delta(0);
   return (norm_lot(lot));
}
...
double prev_lot(double lot = 0.0) {
   lot = min_risk * AccountBalance() / MarketInfo(Symbol(), MODE_MARGINREQUIRED);
   return (norm_lot(lot));
}
