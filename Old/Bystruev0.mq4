//+------------------------------------------------------------------+
//|                                                     Bystruev.mq4 |
//|                    Copyright © 2009, Denis Bystruev, 13 Feb 2009 |
//|                                           http://www.moeradio.ru |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2009, Denis Bystruev, 13 Feb 2009"
#property link      "http://www.moeradio.ru"

string   MySymbol;         // currency symbol, e. g. EURUSD
double   Delta = 0.0005;   // min difference between current price and order price
      ...
      case 2:              // two orders
         if (BuyOrder > 0)          // at first work with the buy order
         {
            if (OrderSelect(BuyOrder, SELECT_BY_TICKET))
            {
               ...
               } else if (BuyPrice + Delta < Bid)    // bid went up high
               {
                  ...
                  if (BuyStopLoss < NewStopLoss)         // need to change BuyStopLoss
                  {
                     ...
                     OrderModify(BuyOrder, BuyPrice, BuyStopLoss, 0.0, 0.0);
                  }
         ...
         if (SellOrder > 0)         // then we work with the sell order
         {
            if (OrderSelect(SellOrder, SELECT_BY_TICKET))
            {
               ...
               } else if (Ask + Delta < SellPrice)   // ask went down low
               {
                  ...
                  if (NewStopLoss < SellStopLoss)        // need to change SellStopLoss
                  {
                     ...
                     OrderModify(SellOrder, SellPrice, SellStopLoss, 0.0, 0.0);
                  }
